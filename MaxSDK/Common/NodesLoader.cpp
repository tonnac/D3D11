...
void NodesLoader::LoadMesh(INode* node, ZXCObject* o)
{
	...
	{
		o->mSkinTri.resize(mesh.getNumFaces());
		std::vector<BipedVertex> bv;
		LoadBipedInfo(node, bv, o->mNodeName.first);
		for (int i = 0; i < (int)o->mSkinTri.size(); ++i)
		{
			LoadVertex(mesh, tm, &o->mSkinTri[i], i, i0, i1, i2);

			if (bv.empty())
			{
				o->mSkinTri[i].v[0].w[0] = 1.0f;
				o->mSkinTri[i].v[0].i[0] = (int)o->mNodeName.second;
				o->mSkinTri[i].v[0].w[3] = 1.0f;

				o->mSkinTri[i].v[1].w[0] = 1.0f;
				o->mSkinTri[i].v[1].i[0] = (int)o->mNodeName.second;
				o->mSkinTri[i].v[1].w[3] = 1.0f;

				o->mSkinTri[i].v[2].w[0] = 1.0f;
				o->mSkinTri[i].v[2].i[0] = (int)o->mNodeName.second;
				o->mSkinTri[i].v[2].w[3] = 1.0f;
			}
			else
			{
				if (o->mSkinTri[i].v[0].VertexNum >= (int)bv.size() ||
					o->mSkinTri[i].v[1].VertexNum >= (int)bv.size() ||
					o->mSkinTri[i].v[2].VertexNum >= (int)bv.size())
				{
					if (o->mSkinTri[i].v[0].VertexNum >= (int)bv.size())
					{
						o->mSkinTri[i].v[0].w[0] = 1.0f;
					...
					{
						InputBipedes(o->mSkinTri[i].v[0], bv[o->mSkinTri[i].v[0].VertexNum]);
					}

					if (o->mSkinTri[i].v[1].VertexNum >= (int)bv.size())
					{
						o->mSkinTri[i].v[1].w[0] = 1.0f;
						o->mSkinTri[i].v[1].i[0] = (int)o->mNodeName.second;
						o->mSkinTri[i].v[1].w[3] = 1.0f;
					...
					{
						InputBipedes(o->mSkinTri[i].v[1], bv[o->mSkinTri[i].v[1].VertexNum]);
					}

					if (o->mSkinTri[i].v[2].VertexNum >= (int)bv.size())
					{
						o->mSkinTri[i].v[2].w[0] = 1.0f;
						o->mSkinTri[i].v[2].i[0] = (int)o->mNodeName.second;
						o->mSkinTri[i].v[2].w[3] = 1.0f;
					...
					{
						InputBipedes(o->mSkinTri[i].v[2], bv[o->mSkinTri[i].v[2].VertexNum]);
					}
				}
				else
				{
...
template<typename X>
void NodesLoader::LoadVertex(const Mesh & mesh, const Matrix3 & tm, X * t, int i, std::uint32_t i0, std::uint32_t i1, std::uint32_t i2)
{
	Point3 v;
...
template<typename X>
void NodesLoader::LoadTexCoord(const Mesh & mesh, const Matrix3 & tm, X* t, int i, std::uint32_t i0, std::uint32_t i1, std::uint32_t i2)
{
...
template<typename X>
void NodesLoader::LoadColor(const Mesh & mesh, const Matrix3 & tm, X* t, int i, std::uint32_t i0, std::uint32_t i1, std::uint32_t i2)
{
...
template<typename X>
void NodesLoader::LoadNormal(Mesh & mesh, const Matrix3 & tm, X* t, int i, std::uint32_t i0, std::uint32_t i1, std::uint32_t i2)
{
	mesh.buildNormals();
...
void NodesLoader::InputBipedes(SkinnedVertex & vertex, const BipedVertex & bipedes)
{
	int i = 0;
...
NodesLoader::NodesLoader(BaseExporter * exporter, bool isSkinned)
	: mExporter(exporter), mIsSkinned(isSkinned)
{
}
