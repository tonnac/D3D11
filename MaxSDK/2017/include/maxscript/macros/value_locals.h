...
/*! Class that uses the RAII idiom to push/pop the thread local current_locals_frame. 
	...
	and value_local_array macros below.
...
class MXS_Exit_ValueLocal_Scope : public MaxSDK::Util::Noncopyable 
{
	Value** frame;
public:
	ScripterExport MXS_Exit_ValueLocal_Scope(bool validateFrame = true);
...
#pragma deprecated("pop_value_local_array")
#pragma deprecated("pop_value_locals")
...
// all functions that return a Value* should return via return_value
#define return_value(r)	{ return templated_return_value_fn(r); }

//! \deprecated Deprecated as of 3ds Max 2017. Use return_value instead. value local storage handled by MXS_Exit_ValueLocal_Scope instances
...
public:
	...
	ScripterExport ScopedValueTempArray(unsigned int count = 0);
...
public:
	...
	~ScopedMaxScriptSEHandler()
	{
		_set_se_translator( m_oldTranslatorFunction );
	}
...
public:
	...
	ScripterExport ScopedMaxScriptEvaluationContext(bool initThreadLocals = true);
...
public:
	ScripterExport ScopedPushControl(Control* c);
