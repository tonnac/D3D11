...
Computeshader::Computeshader(ID3D11Device * pd3Device, UINT width, UINT height, DXGI_FORMAT format, const std::wstring& shaderFile)
...
void Computeshader::OnResize(UINT width, UINT height)
{
...
void Computeshader::Execute(ID3D11DeviceContext * context, ID3D11ShaderResourceView** texture)
{
...
ID3D11ShaderResourceView ** Computeshader::ShaderResrouceView()
{
	return mSrv.GetAddressOf();
}

void Computeshader::BuildShader(const std::wstring& shaderFile)
{
	std::wstring name(shaderFile, shaderFile.find_last_of('\\') + 1,  shaderFile.find_last_of('.') - shaderFile.find_last_of('\\') - 1);
...
void Computeshader::BuildResource()
{
	mTexture.Reset();
