...
{
	return SceneInf.FirstFrame * SceneInf.FrameTick;
}

int AnimationClip::GetClipEndTime()const
{
	return SceneInf.LastFrame * SceneInf.FrameTick;
}

void AnimationClip::Interpoloate(int t, std::vector<XMFLOAT4X4>& boneTransforms, const std::vector<XMFLOAT4X4>& Initpos)const
{
	...
	std::vector<XMFLOAT4X4>& finalTransforms)const
{
	...
	auto clip = mAnimations.find(clipName);
	clip->second.Interpoloate(timePos, toParentTransforms, mInitPos);
...
void SkinnedData::GetFrameTick(const std::wstring & name, UINT & speed, UINT & tick)
{
	speed = mAnimations[name].SceneInf.FrameSpeed;
	tick = mAnimations[name].SceneInf.FrameTick;
}
