...
MeleeAttack::MeleeAttack(Player* pPlayer) : PlayerState(pPlayer) ,m_pEffect(nullptr)
{}

PlayerAttack::PlayerAttack(Player * pPlayer) : MeleeAttack(pPlayer)
{
	m_pCharacter->AddState(std::tstring(L"Attack1"), this);
	m_pEffect = new KahoAttack1;
}
bool PlayerAttack::Init()
{
	setSprite(L"Kaho", L"Attack1");
	m_pSprite->setDivideTime(0.7f);
	m_pEffect->LoadFile(L"PLAYER", L"../../../../data/bmp/KahoColor.bmp", L"../../../../data/bmp/KahoMask.bmp");
	m_pEffect->Init();
	return true;
}
bool PlayerAttack::Frame()
{
	m_CenterPos->x += m_pCharacter->getDir() * g_fSecPerFrame * 30.0f;

	m_fTimer += g_fSecPerFrame;
	FloatPoint pt;
	if (m_pCharacter->getDir() == -1)
	{
		m_pEffect->setRendering(2.8f, INVERSE::LR_ROTATION);
		pt.x = m_CenterPos->x - 75.0f;
		pt.y = m_CenterPos->y;
	}
	else
	{
		m_pEffect->setRendering(2.8f, INVERSE::DEFAULT);
		pt.x = m_CenterPos->x + 75.0f;
		pt.y = m_CenterPos->y;
	}
	if (m_fTimer >= 0.4f && S_Input.GetKey('S') == KEYSTATE::KEY_PUSH)
	{
		...
		m_pEffect->setIndex(0);
		m_fTimer = 0.0f;
		m_pCharacter->setState(L"Attack2");
		return true;
	}
	m_pEffect->Frame();
	m_pEffect->setDrawPos(pt);
	if (!m_pSprite->Frame())
	{
		m_pSprite->setIndex(0);
		m_pEffect->setIndex(0);
...
{
	m_pCharacter->addState(std::string("Attack2"), this);
	m_pEffect = new KahoAttack2;
}
...
bool PlayerAttack2::Frame()
{
	...
	m_pEffect->setDebugmode(m_pCharacter->getDebugmode());
	m_fTimer += g_fSecPerFrame;
	FloatPoint pt;
	if (m_pCharacter->getDir() == -1)
	{
...
{
	m_pCharacter->addState(std::string("Attack3"), this);
	m_pEffect = new KahoAttack3;
}
...
{
	m_pCharacter->addState(std::string("AirAttack"), this);
	m_pEffect = new KahoAirAttack;
}
bool AirAttack::Init()
{
...
{
	m_pCharacter->addState(std::string("Bow"), this);
}
bool BowAttack::Init()
{
...
{
	m_pCharacter->addState(std::string("AirBow"), this);
}
...
{
	m_pCharacter->addState(std::string("CrouchBow"), this);
}
