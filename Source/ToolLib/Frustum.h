...
struct boundingBox
{
	DirectX::XMFLOAT3 mCenter = DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f);
	std::array<DirectX::XMFLOAT3, 3> mAxis;
	std::array<float, 3> mExtent;
};
...
public:
	...
	bool Contains(const DirectX::XMFLOAT3& point);
	bool Contains(const DirectX::XMFLOAT3 & center, float radius);
