...
bool Object::Frame()
{
	//m_rtCollision = { m_VertexList[0].TexPos.x, m_VertexList[0].TexPos.y, 0.0f, 0.0f };
	//for (int VertexSize = 0; VertexSize < m_VertexList.size(); ++VertexSize)
	//{
	//	m_rtCollision.left = DirectX::XMMin(m_rtCollision.left, m_VertexList[VertexSize].TexPos.x);
	//	m_rtCollision.top = DirectX::XMMin(m_rtCollision.top , m_VertexList[VertexSize].TexPos.y);
	//	m_rtCollision.right = DirectX::XMMax(m_rtCollision.right, m_VertexList[VertexSize].TexPos.x);
	//	m_rtCollision.bottom = DirectX::XMMax(m_rtCollision.bottom, m_VertexList[VertexSize].TexPos.y);
	//}

	//D2D1_POINT_2F f2 = { (m_rtCollision.right) * 4.0f, (m_rtCollision.bottom) * 4.0f};
	//ComputeCollision(f2);
	//D2D1_POINT_2F Centeroe = ComputeCoord();
	//m_VertexList[0].Pos.x = Centeroe.x - (m_rtCollision.right - m_rtCollision.left) * 0.5f / 800.0f;
	//m_VertexList[0].Pos.y = Centeroe.y - (m_rtCollision.bottom - m_rtCollision.top) * 0.5f / 600.0f;
	//m_VertexList[1].Pos.x = Centeroe.x + (m_rtCollision.right - m_rtCollision.left) * 0.5f / 800.0f;
	//m_VertexList[1].Pos.y = Centeroe.y - (m_rtCollision.bottom - m_rtCollision.top) * 0.5f / 600.0f;
	//m_VertexList[2].Pos.x = Centeroe.x - (m_rtCollision.right - m_rtCollision.left) * 0.5f / 800.0f;
	//m_VertexList[2].Pos.y = Centeroe.y + (m_rtCollision.bottom - m_rtCollision.top) * 0.5f / 600.0f;
	//m_VertexList[3].Pos.x = Centeroe.x + (m_rtCollision.right - m_rtCollision.left) * 0.5f / 800.0f;
	//m_VertexList[3].Pos.y = Centeroe.y + (m_rtCollision.bottom - m_rtCollision.top) * 0.5f / 600.0f;
...
void Object::CreateRasterizer(ID3D11Device* pDevice)
{
